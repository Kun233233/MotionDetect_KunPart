...
////												 };
//
////__global__ void MultiplyKernel(const float* depth, const float* pixels_to_points_map, float* points, int size) {
////	int idx = threadIdx.x + blockIdx.x * blockDim.x;
////	if (idx < size) {
////		points[idx] = depth[idx] * pixels_to_points_map[idx];
////	}
////}
//
//
...
//
//
//cv::Mat Get3DPoints_cuda(const cv::cuda::GpuMat& depth_C, const const cv::cuda::GpuMat& pixels_to_points_map_C)
//{
...
//	cv::cuda::GpuMat depth_flatten = depth_C.reshape(1, 1);
//
...
//
//			// 将读取的rgb和depth转换至GPU上
//			cv::cuda::GpuMat rgb_C, depth_C;
//			rgb_C.upload(rgb);
//			depth_C.upload(depth);
//
//
